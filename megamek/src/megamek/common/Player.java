...
public final class Player extends TurnOrdered implements IPlayer {
    ...
    public Vector<Integer> getAirborneVTOL() {
        ...
        return game.getEntitiesVector().stream()
                .filter(entity -> entity.getOwner().equals(this))
                .filter(entity -> (entity instanceof VTOL || entity.getMovementMode() == EntityMovementMode.WIGE))
                .filter(entity -> !entity.isDestroyed())
                .filter(entity -> entity.getElevation() > 0)
                .map(Entity::getId)
